<template>
  <div>
    ...
    <p>
      ...
      手数を節約して4つのグッドエッジとCOを作るのに最良の方法は、この2つを同時に行うことです。 発生しうるケースは2つあります。
      Eエッジとコーナーを同時に処理することを、ドミノリダクショントリガー（DRトリガー）と呼びます。
      コーナーだけ、E-エッジだけ、または両方できているがEOが揃っていない場合は、部分的ドミノリダクション（PDR）と呼びます。
    </p>
    <h3>2.1 DRトリガー</h3>
    <p>
    ...
    <p>
      ...
      U/D色のコーナーとエッジが3つ並んでいる部分（今回の場合、F面右）を「バー」、
      E層エッジの上下コーナーの柱断面にあたる部分がU/D色になっている部分（上面左）を「カラム」と呼びます。<br>
      （訳注：原文が非常に英語的な表現で分かりづらいため、訳者なりの解釈で記述しています）
    </p>
    <p>
